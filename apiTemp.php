...
$previsionDefaultImage = BASE_URL . 'dermalogo.png';
...
=======
        $strSQL = "SELECT * FROM doctors WHERE expert = 1 and id <> '$userid' ORDER BY doctors.time DESC LIMIT $_offset, $_limit";
>>>>>>> 8c203d9b9e68374f8e0c3caa047353d68d3391c8
    ...
    case 'consult_message_details':
        ...
        if (!$q) {
            ...
            if (mysql_num_rows($q)) {
                ...
                if( $row['image'] != '' ){
                    if( $account['doctor_login_source'] == 'ios'){
                        $image = '<a style="color:#03F" onclick="window.open(\''.$_SITE_PATH_ . 'uploads/consult/' .$row['image'].'\',\'_blank\');" >Read More...</a>';
                    }else{
                ...
                if( $row['file'] != '' ){
                    if( $account['doctor_login_source'] == 'ios'){
                        $link = '<a style="color:#03F" onclick="window.open(\''.$_SITE_PATH_ . 'uploads/consult/' .$row['file'].'\',\'_blank\');" >Read More...</a>';
                    }else{
                ...
                $response['data'] = array(
                    ...
                    'userid' => $row['user_id'],
                    'description' => $row['description'].'<br><br>'.$link.$image,
                );
            }
        }
        break;
    ...
    case 'consult_inbox_messages':
        ...
        */
        ...
        if (!$q) {
            ...
            if (mysql_num_rows($q)) {
                while ($row = mysql_fetch_assoc($q)) {
                    $name = ($row['mention_name'] ) ? $row['expname'] : "An Expert";
                    $data = array(
                        'id' => $row['id'],
                        'title' => $row['title'],
                        'description' => summarize($row['description'], 10),
...
function getPushNotifications($account) {
    ...
    $strSQL = "SELECT *  FROM `notifications` WHERE `notifications`.show = 1 AND user_id ='".$account['id']."' ORDER BY id DESC";
    $q = mysql_query($strSQL);
    ...
    if (mysql_num_rows($q)) {
        ...
        $strSQL2 = "SELECT * FROM `notifications` WHERE user_id ='".$account['id']."' ORDER BY id DESC LIMIT 10" ;
    ...
    return array('notifications'=>$notifications, 'all'=>$allCount, 'news'=>$newsCount, 'medical'=>$medicalCount, 'consult'=>$consultCount,'experts'=>$expertsCount);
