...
define('THUMB_WIDTH', 200); // 218
define('THUMB_HEIGHT', 200); // 150
...
$fieldsArray = array(
	...
	'info_phone', 'info_mobile', 'info_email',  'info_address', 'expert',
	'hospital', 'specialization', 'date', 'brief'
);
$csvOptions = array(
	'delimiter' => ';',
	'fields' => array(
		...
		<div class="module_content inline">
				...
				<?php } ?>
						...
						<tr>
							...
							<td>
                                <label>Hospital
                                    <br /><input type="text" name="hospital[]" value="<?php echo textencode($hospital[$oldrecord[$i]]); ?>" />
                                </label>
							</td>
							<td>
                                <label>Specialization
                                    <br /><input type="text" name="Specialization[]" value="<?php echo textencode($specialization[$oldrecord[$i]]); ?>" />
                                </label>
							</td>
						</tr>
						<tr>
                            <td>
                                <label>Birth Date
                                    <br /><input type="text" name="date[]" value="<?php echo textencode($date[$oldrecord[$i]]); ?>" />
                                </label>
                            </td>							
						</tr>
					</table>
                ...
                <fieldset>
					<label>Brief</label>
                    <textarea name="brief[]" rows="2" ><?php echo textencode($brief[$oldrecord[$i]]); ?></textarea>
				</fieldset>
				<fieldset>
					<label>Notes</label>
					<textarea name="description[]" rows="12" ><?php

						//$string = html_entity_decode(textencode($description[$oldrecord[$i]]));
						//$string = preg_replace("/\s/",'',$string);
						echo  $description[$oldrecord[$i]]; 
				...
				<fieldset>
					...
					<?php file_field('file1'.$i,'../uploads/doctors/',$file1[$oldrecord[$i]]);?>
                                ...
                                <fieldset>
                                    <label>Set as Expert</label>
                                    <select name="expert[]">
                                        <option value="1" <?php echo selected($status[$oldrecord[$i]], '1'); ?>> Yes </option>
                                        <option value="0" <?php echo selected($status[$oldrecord[$i]], '', isset($status[$oldrecord[$i]])); ?>> No </option>
                                    </select>
				</fieldset>
				<div class="clear"></div>
		</div>
...
<?php
	...
	default:
		...
		if( $action == 'unchanged_passwords'  || $action == 'pre_print') {
			$PageSize = 10*1000;
...
<div class="alert_browse">
	<form action="<?php echo $filename; ?>" method="GET" >
<?php if( $action == 'unchanged_passwords') { ?>
...
<!--<?php 
		...
		if( !empty($keyword) )
		{
			...
			OR doctors.email LIKE '%$keywordSQL%' ) ";
		}
...
//				$sql[] = " doctors.status='' ";
		...
		$objRS=mysql_query($strSQL);
		$count_all_all = @mysql_num_rows($objRS);
...
<?php } else { ?>
	<ul class="tabs">
		<li><a href="#Export" onClick="window.location='<?php echo $filename; ?>?<?php echo $queryStr; ?>p=<?=$p?>&action=pre_print'" >Export List</a></li>
	</ul>
	<ul class="tabs">
...
?>
		<td>
			...
			<div><b>login time</b>: <?php if ($row->time_update) echo date('d/m/Y H:i:s', $row->time_update); ?></div>
		</td>
<?php 
		$TDs++;
		if(!($TDs%2)) {
...
<?php } else { ?>
		...
		<thead> 
			...
			<?php } ?>
				...
				<th width="1" >Device <?php /* echo "<br>android(". $android. ")  ios(" . $ios . ")"; */ ?></th>
				<th width="1">login date</th>		 
    			<th width="1">City (<?php echo $total_cities ."/" .$total_categs ;  //mysql_num_rows($objRS);  ?>) </th>
	...
	<?php } ?>
	  ...
	  <td>
		...
		<td>
			<b><?php if ($row->time_update) echo date('d/m/Y H:i:s', $row->time_update); ?></b>
		</td>
		<td>
