...
//ALTER TABLE `expert_articles` ADD `publish_date_time` DATETIME NOT NULL AFTER `app_notification` ;
$relatedArray = array(
//	'doctors' => " doctors ",
...
$sortableLister = new lister("_sort_reverse.php?filename=" . $filename . "&table=expert_articles");

$rank = getfield(getHTTP('rank'), "rank", "expert_articles");
...
switch ($action):
    //case "down":
    case "up": //reverse
        $strSQL = "select * from expert_articles WHERE rank='" . ($rank + 1) . "'";
        $objRS = mysql_query($strSQL);
        $total = mysql_num_rows($objRS);
        if ($total > 0) {
            if ($row = mysql_fetch_object($objRS)) {
                $strSQLord = "update expert_articles set rank='" . ($rank + 1) . "' WHERE rank='" . $rank . "'";
                mysql_query($strSQLord);
                $strSQLord = "update expert_articles set rank='" . $rank . "' WHERE id='" . $row->id . "'";
                mysql_query($strSQLord);
            }
        } else {
            $strSQL = "select * from expert_articles WHERE rank >'" . $rank . "'";
            $objRS = mysql_query($strSQL);
            $total = mysql_num_rows($objRS);
            if ($total > 0) {
                if ($row = mysql_fetch_object($objRS)) {
...
//							$Rimage->resize( THUMB_WIDTH, THUMB_HEIGHT);
                    ...
                    if ($result = file_upload('document', 'file2' . $i, '../uploads/expert_articles/', $errorMsg[$j])) { // document
                        $file2[$i] = $result['name'];
                    }
...
//		if($j>0){
    ...
    case "delete":
        ...
        while ($row = mysql_fetch_object($objRS)) {
            ...
            if ($row->document != '') {
                @unlink('../uploads/expert_articles/' . $row->document);
            }
        ...
        <?php for ($i = 0; $i < $max; $i++) { ?>
            ...
            ?>
							...
							<fieldset>
                                ...
                                <?php file_field('file2' . $i, '../uploads/expert_articles/', $file2[$oldrecord[$i]]); ?>
                            </fieldset>
...
//echo "$strSQL " . mysql_error();
        ...
        ?>
                ...
                <form action="<?php echo $filename; ?>?<?php echo $queryStr; ?>p=<?= $p ?>&action=delete" method="post" name="del" id="del">
                    <article class="module width_full">
                        <header><h3 class="tabs_involved">Expert Articles Manager</h3>
                            <ul class="tabs">
        ...
        <?php while ($row = mysql_fetch_object($objRS)) { ?>
            ...
            <?php } ?>
                            ...
                            ?>
                                            ...
                                            <td align="center"><?php
                                                if ($row->document) {
                                                    echo "<a href=\"#\" onclick=\"window.open('../uploads/expert_articles/{$row->document}','','width=500,height=400,scrollbars,resizable')\"><img src=\"images/view.gif\" border=\"0\"></a> ";
                                                }
                                                ?></td>
