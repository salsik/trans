...
switch ($action):
    ...
    case "addexe":
        ...
        for ($i = 0; $i < sizeof($ids); $i++) {
            ...
            $sql = array();
            foreach ($fieldsArray as $field) {
                //$sql[$field] = " `$field`='" . sqlencode(trime(${$field}[$i])) . "', ";htmlspecialchars($sql['description'])
                $sql[$field] = " `$field`='" . htmlspecialchars(addslashes(${$field}[$i])) . "', ";
            }
            ...
            if ($flag) {
                ...
                if ($q) {
                    ...
                    if (mysql_errno() == 1062) {
                        ...
                        $errorMsg[$j] = 'Something went wrong! '.mysql_error();
                    }
        ...
        <?php for ($i = 0; $i < $max; $i++) { ?>
            ...
            ; ?>
                        ...
                        <fieldset>
                            ...
                            <textarea name="about[]" rows="12" ><?php echo $about[$oldrecord[$i]]; ?></textarea>
                        </fieldset>
                        <fieldset>
                            <label>Products</label>
                            <textarea name="products[]" rows="12" ><?php echo $products[$oldrecord[$i]]; ?></textarea>
                        </fieldset>                        
                        <fieldset>
...
//				$sql[] = " resellers.status='' ";
        ...
        if ($action == 'export') {
            ...
            $excel->addField('about', 'About');
            $excel->addField('products', 'products');
