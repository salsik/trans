...
//ALTER TABLE `products` ADD `publish_date_time` DATETIME NOT NULL AFTER `app_notification` ;
$relatedArray = array(
//	'doctors' => " doctors ",
//	'products' => " products ",
//	'documents' => " documents ",
...
$sortableLister = new lister("_sort_reverse.php?filename=" . $filename . "&table=products");

$rank = getfield(getHTTP('rank'), "rank", "products");
...
switch ($action):
    //case "down":
    case "up": //reverse
        $strSQL = "select * from products WHERE rank='" . ($rank + 1) . "'";
        $objRS = mysql_query($strSQL);
        $total = mysql_num_rows($objRS);
        if ($total > 0) {
            if ($row = mysql_fetch_object($objRS)) {
                $strSQLord = "update products set rank='" . ($rank + 1) . "' WHERE rank='" . $rank . "'";
                mysql_query($strSQLord);
                $strSQLord = "update products set rank='" . $rank . "' WHERE id='" . $row->id . "'";
                mysql_query($strSQLord);
            }
        } else {
            $strSQL = "select * from products WHERE rank >'" . $rank . "'";
            $objRS = mysql_query($strSQL);
            $total = mysql_num_rows($objRS);
            if ($total > 0) {
                if ($row = mysql_fetch_object($objRS)) {
    ...
    case "edit":
        ...
        $strSQL = "select * from products where id IN(" . implode(',', $ids) . ") $limitation order by rank DESC";
        $objRS = mysql_query($strSQL);
        $i = 0;
    ...
    case "addexe":
        ...
        for ($i = 0; $i < sizeof($ids); $i++) {
            ...
            $strSQL = "products set " . implode('', $sql);
            ...
            if ($action == "editexe"):
                ...
                if ($Errors) {
                    ...
                    if ($result = file_upload('image', 'file1' . $i, '../uploads/products/', $errorMsg[$j])) {
                        $file1[$i] = $result['name'];
                        if (empty($errorMsg[$j])) {

                            $Rimage = new SimpleImage();
                            $Rimage->load('../uploads/products/' . $file1[$i]);
                            if ($Rimage->getWidth() > THUMB_WIDTH) {
                                $Rimage->resizeToWidth(THUMB_WIDTH);
                            }
//							$Rimage->resize( THUMB_WIDTH, THUMB_HEIGHT);
                            $Rimage->save('../uploads/products/thumb/' . $file1[$i]);
                        }
                    }


                    if ($result = file_upload('document', 'file2' . $i, '../uploads/products/', $errorMsg[$j])) { // document
                        $file2[$i] = $result['name'];
                    }
...
//							$Rimage->resize( THUMB_WIDTH, THUMB_HEIGHT);
                    ...
                    if (!empty($errorMsg[$j])) {// Upload Error
                        ...
                        $q = mysql_query("SELECT max(rank) as max FROM products");
            ...
            if ($flag) {
                ...
                if ($q) {
                    ...
                    setUpdatedRow('products', $news_id, $_action);
    ...
    case "delete":
        ...
        while ($row = mysql_fetch_object($objRS)) {
            ...
            if ($row->image != '') {
                ...
                @unlink('../uploads/products/thumb/' . $row->image);
            }


            if ($row->document != '') {
                @unlink('../uploads/products/' . $row->document);
            }



            $strSQL = "DELETE FROM products WHERE id = '{$row->id}' LIMIT 1";
            if (!mysql_query($strSQL)) {
                if (empty($errorMsg)) {
                    $errorMsg = "Some Records didn't affected!!";
                }
            } else {
                setUpdatedRow('products', $row->id, 'delete');
    ...
    case "record":
        ...
        <?php for ($i = 0; $i < $max; $i++) { ?>
            ...
            ?>
                                        ...
                                        Send this product in push notification.
                                    </label>
                                    <div class="clear"></div>
                            </fieldset>
            <?php
            ...
            ?>
                          ...
                          <!--   <fieldset>
                                ...
                                <?php file_field('file2' . $i, '../uploads/products/', $file2[$oldrecord[$i]]); ?>
                            </fieldset> -->
...
EOF;
                            ...
                            <fieldset>
                                <label>Image (Min&nbsp;Width:&nbsp;<?php echo THUMB_WIDTH; ?>px)</label>
                                            <?php file_field('file1' . $i, '../uploads/products/', $file1[$oldrecord[$i]]); ?>
                            </fieldset>
                            <div class="clear"></div>
                        </div>
                        <footer></footer>
        ...
        if (!empty($keyword)) {
            ...
            $sql[] = " (products.title LIKE '%$keywordSQL%' OR products.description LIKE '%$keywordSQL%' ) ";
        }
...
//				$sql[] = " products.status='' ";
		...
		FROM ( ( products
		LEFT JOIN news_index ON (products.id = news_index.index_id)
		LEFT JOIN category ON (category.id = news_index.cat_id )
		) 
		LEFT JOIN news_resellers ON (products.id = news_resellers.news_id) )
		LEFT JOIN news_doctors ON (products.id = news_doctors.news_id)
...
//		LEFT JOIN doctors ON (doctors.id = doctors_resellers.doctor_id AND news_doctors.doctor_id )
        ...
        if ($action == 'export') {
            ...
            $excel = new export_excel("Expert Articles");
            ...
            $excel->export("$strSQL ORDER BY products.rank DESC");
            exit;
        }
...
//echo "$strSQL " . mysql_error();
        $total = @mysql_num_rows($objRS);
        $strSQL = makePages($strSQL, $PageSize, $p, 'products.rank desc');
        $objRS = mysql_query($strSQL);
        ...
        <?php while ($row = mysql_fetch_object($objRS)) { ?>
                                        <tr id="tr_<?php echo $row->id; ?>">
                                            ...
                                            <td align="center"><?php echo scale_image("../uploads/products/thumb/" . $row->image, 100); ?></td>
                                            <td>
            ...
            <?php } ?>
                            ...
                            ?>
                                            ...
                                            <td align="center"><?php
                                                if ($row->document) {
                                                    echo "<a href=\"#\" onclick=\"window.open('../uploads/products/{$row->document}','','width=500,height=400,scrollbars,resizable')\"><img src=\"images/view.gif\" border=\"0\"></a> ";
                                                }
                                                ?></td>
