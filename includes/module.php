...
function textencode($strng,$frontHTML='0'){
 $from=array('&amp;#','&amp; ','&amp;');
 $to=array('&#','&amp;','&amp; ');
 $temp=str_replace($from,$to,htmlspecialchars($strng));
 if(!$frontHTML)
	 return $temp;
 else 
	 return nl2br($temp);
...
function sqlencode($strng){
	$strng = htmlspecialchars($strng, ENT_QUOTES, 'UTF-8', false);
...
//sendgrid mail
function sendgridmail($fromname,$frommail,$to,$subject,$message)
{
	
require("sendgrid-php/sendgrid-php.php");


$_REQUEST['contact_name']  = $fromname ;
$_REQUEST['contact_email']  = $frommail ;



$from = new SendGrid\Email($_REQUEST['contact_name'], $_REQUEST['contact_email']);
...
$apiKey = "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" ;// getenv('SENDGRID_API_KEY');
