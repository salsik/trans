...
//							}
                    ...
                    if ( $action == 'addexe' && $_POST['app_notification'][0] == 1 ) {
						...
						// flag for send to transplantation
                        ...
                        if( count($_doctors) > 0 ){
							...
							// if its transplantation company .. we should send them their own notifications
					...
					//	send them their own notifications
						if ($transplantation)
						{
							$dataToSend = array();
									
									$data1= array('type'=>'news', 'id'=>$news_id, 'title'=>  $_POST['title'][0] );
...
//				$sql[] = " news.status='' ";
		...
		LEFT JOIN news_doctors ON (news.id = news_doctors.news_id)
