...
$sortableLister = new lister("_sort_reverse.php?filename=".$filename."&table=documents_category");

$rank=getfield(getHTTP('rank'),"rank","documents_category");

switch ($action):
	//case "down":
		case "up": //reverse
		$strSQL="select * from documents_category WHERE rank='".($rank+1)."'" ;
		$objRS=mysql_query($strSQL);
		$total=mysql_num_rows($objRS);
		if($total>0){
			if ($row=mysql_fetch_object($objRS)){
				$strSQLord="update documents_category set rank='".($rank+1)."' WHERE rank='".$rank."'";
				mysql_query($strSQLord);
				$strSQLord="update documents_category set rank='".$rank."' WHERE id='".$row->id."'";
				mysql_query($strSQLord);
			}
		}
		else{
			$strSQL="select * from documents_category WHERE rank >'".$rank."'";
			$objRS=mysql_query($strSQL);
			$total=mysql_num_rows($objRS);
			if($total>0){
				if ($row=mysql_fetch_object($objRS)){
	...
	case "edit":
		...
		$strSQL="select * from documents_category where id IN(".implode(',',$ids).") order by rank DESC";
		$objRS=mysql_query($strSQL);
		$i=0;
		...
		for($i=0;$i<sizeof($ids);$i++){
			...
			if ($action=="editexe"):
				...
				if ( $Errors ){
					...
					if ( $result=file_upload('image','file1'.$i,'../uploads/documents_category/',$errorMsg[$j])){
						 $file1[$i]=$result['name'];
						if(empty($errorMsg[$j])){

							$Rimage = new SimpleImage();
							$Rimage->load('../uploads/documents_category/'.$file1[$i]);
							if( $Rimage->getWidth() > THUMB_WIDTH ) {
								$Rimage->resizeToWidth( THUMB_WIDTH );
							}
//							$Rimage->resize( THUMB_WIDTH, THUMB_HEIGHT);
							$Rimage->save('../uploads/documents_category/thumb/'.$file1[$i]);							
						}						 
					}
...
//							$Rimage->resize( THUMB_WIDTH, THUMB_HEIGHT );
					...
					if(!empty($errorMsg[$j])){// Upload Error
						...
						$q=mysql_query("SELECT max(rank) as max FROM documents_category");
...
//		if($j>0){
	...
	case "delete":
		...
		while ($row=mysql_fetch_object($objRS))
		{
			$q = mysql_query("SELECT * FROM `documents` WHERE documents_cat_id = '".$row->id ."' LIMIT 1");
			if( $q && mysql_num_rows($q) ) {
				if( empty($errorMsg) ) {
					$errorMsg = "Some Records didn't affected, You may need to delete/update all documents for schools related to the deleted category!!";
				}
				continue;
			}

			if($row->image != '') {
				@unlink('../uploads/documents_category/'.$row->image);	
				@unlink('../uploads/documents_category/thumb/'.$row->image);	
			}

			$strSQL="DELETE FROM documents_category WHERE id = '".$row->id ."' LIMIT 1";
...
<article class="module width_full">
	<header><h3>Documents Categories: <?php echo ucwords($action); ?> Record
...
<?php } ?>
				...
				<fieldset>
					<label>Image</label>
					<?php file_field('file1'.$i,'../uploads/documents_category/',$file1[$oldrecord[$i]]);?>
				</fieldset>
				<div class="clear"></div>
		</div>
	<footer></footer>
...
<?php 
		...
		if( !empty($keyword) )
		{
			...
			$sql[] = " (documents_category.title LIKE '%$keywordSQL%' OR documents_category.description LIKE '%$keywordSQL%' ) ";
		}
	if( $enableStatus ) {
		switch($_GET['status']) {
			case 'active':
				$sql[] = " documents_category.status='active' ";
				break;
			case 'inactive':
//				$sql[] = " documents_category.status='' ";
				$sql[] = " documents_category.status<>'active' ";
				break;
		}
	}
		...
		$strSQL="SELECT documents_category.* FROM documents_category $where";

		
		if( $action == 'export' ) {
			include 'functions/_export.php';
			
			$excel = new export_excel("Documents Categories");
			$excel->addField('title', 'Title');
			$excel->addField('description', 'Description');
		if( $enableStatus ) {
			$excel->addField('status', 'Status', 'ucwords');
		}
			$excel->export( "$strSQL ORDER BY documents_category.rank DESC");
			exit;
		}
		$objRS=mysql_query($strSQL);
		$total=@mysql_num_rows($objRS);
		$strSQL=makePages ($strSQL,$PageSize,$p, 'documents_category.rank desc');
		$objRS=mysql_query($strSQL);
...
<article class="module width_full">
	<header><h3 class="tabs_involved">Documents Categories Manager</h3>
	<ul class="tabs">
...
<?php } ?>
		<td align="center"><?php echo scale_image("../uploads/documents_category/thumb/". $row->image, 100); ?></td>
