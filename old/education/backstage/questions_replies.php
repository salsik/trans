...
$limitation2 = '';
if(isSchool) {
	$limitation = " AND questions_replies.question_id IN(SELECT id FROM questions WHERE school_id='".isSchool."') ";
	$limitationQuestion = " AND school_id='".isSchool."' ";
}
else if( !isAdmin ) {
	die();
}
...
$Question = getDataByID('questions', $_GET['question_id'], " TRUE $limitationQuestion");
if( $Question ) {
	$queryStr .= "question_id={$Question['id']}&";
	$limitation2 = " AND questions_replies.question_id='{$Question['id']}' ";
	

	$School = getDataByID('schools', $Question['school_id']);
	$User = getDataByID('users', $Question['user_id']);
}
...
switch ($action):
	...
	case "editexe":
		...
		for($i=0;$i<sizeof($ids);$i++){
			...
			$sql['from'] = " `from`='school', ";
			if( $action == 'addexe' ) {
				$sql['question_id']= " `question_id`='{$Question['id']}', ";
			}
			$sql['school_id']= " `school_id`='{$Question['school_id']}', ";
	...
	<div class="tab_container">
		...
		<tr>
			...
			<td>
				...
				<br />
				<div>
					<b>Related Students:</b>
				</div>
			<?php 
				
				$relatedStudents = array();
				$user_username = mysql_real_escape_string($User['username']);
				$sql = "SELECT * 
					FROM students 
...
<?php 
	...
	$from = ($row->from=='school') ? $School['title']: $User['title'];
...
function style_reply_from($from) {
	...
	return ($from=='school') ? $School['title']: $User['title'];
