...
define('index_students_table', 'documents_students');
define('index_students_field', 'document_id');


$fieldsArray = array(
	'title', 'description', 'status', 'date', 
	'app_notification', 'document_cat_id', 
);
...
$q = mysql_query("SELECT documents_category.* FROM `documents_category` ORDER BY documents_category.rank DESC");
if( $q && mysql_num_rows($q ) ) {
	while( $row = mysql_fetch_assoc( $q )) {
		$documentsCategories[$row['id'] ] = $row;
	}
}

$_GET['document_cat_id'] = intval( $_GET['document_cat_id'] );
if( $documentsCategories[ $_GET['document_cat_id'] ] ) {
	$queryStr .= "document_cat_id={$_GET['document_cat_id']}&";
}
else {
	$_GET['document_cat_id'] = 0;
}
...
switch ($action):
	...
	case "edit":
		...
		while ($row=mysql_fetch_object($objRS)){
			...
			$student_ids[$i] = array();
			$q = mysql_query("SELECT * FROM `documents_students` WHERE document_id='{$row->id}' ");
			if($q && mysql_num_rows($q)) {
				while($indx = mysql_fetch_assoc($q)) {
...
//			$_students = getDataByIDs('students', $students[$i], $limitationStudents);
			...
			else if( ! $documentsCategories[ $document_cat_id[ $i ] ] ) {
				$Errors[] = "Missing Document Category!!";
			}
...
//					if ( $result=file_upload('image','file1'.$i,'../uploads/documents/',$errorMsg[$j])){
//						 $file1[$i]=$result['name'];
//						if(empty($errorMsg[$j])){
//
//							$Rimage = new SimpleImage();
//							$Rimage->load('../uploads/documents/'.$file1[$i]);
//							if( $Rimage->getWidth() > THUMB_WIDTH ) {
//								$Rimage->resizeToWidth( THUMB_WIDTH );
//							}
////							$Rimage->resize( THUMB_WIDTH, THUMB_HEIGHT);
//							$Rimage->save('../uploads/documents/thumb/'.$file1[$i]);							
//						}						 
//					}
					if ( $result=file_upload('documents','file2'.$i,'../uploads/documents/',$errorMsg[$j])){ // document
						 $file2[$i]=$result['name'];
					}
			...
			if($flag){
				...
				if( $q ) {
					if($action == 'addexe') {
						...
						$document_id = intval( $ids[$i] );
					}
					$_action = ( $action == 'addexe' ) ? 'add' : 'edit';
					setUpdatedRow('documents', $document_id, $_action, $document_cat_id[ $i ]);
	...
	case "delete":
		...
		while ($row=mysql_fetch_object($objRS))
		{
			...
			if(!mysql_query($strSQL) ) {
				...
				mysql_query("DELETE  FROM `documents_students` WHERE document_id='{$row->id}' ");
			}
		}
				...
				<fieldset>
					...
					<div style="margin-left: 210px;">
						...
						?>
							...
							Send this document in push notification.
						</label>
						<div class="clear"></div>
				</fieldset>
				<fieldset>
					<label>Document Category</label>
					<select name="document_cat_id[]">
						<option value="" >== Select ==</option>
				<?php foreach($documentsCategories as $documentsCategory) { ?>
						<option value="<?php echo $documentsCategory['id']; ?>" <?php echo selected($document_cat_id[$oldrecord[$i]], $documentsCategory['id']); ?>><?php echo $documentsCategory['title']; ?></option>
				<?php } ?>
					</select>
				</fieldset>
...
<?php if( false ) { ?>
				<fieldset>
					<label>Image</label>
					<?php file_field('file1'.$i,'../uploads/documents/',$file1[$oldrecord[$i]]);?>
				</fieldset>
<?php } ?>
				<div class="clear"></div>
		</div>
...
<div class="alert_browse">
	<form action="<? echo $filename; ?>" method="GET" >
		...
		<div>
			...
			Category: <select name="document_cat_id">
				<option value="" >== Any ==</option>
			<?php foreach($documentsCategories as $documentsCategory) { ?>
				<option value="<?php echo $documentsCategory['id']; ?>" <?php echo selected($_GET['document_cat_id'], $documentsCategory['id']); ?>><?php echo $documentsCategory['title']; ?></option>
			<?php } ?>
			</select>
		</div>
		<div>
...
//				$sql[] = " documents.status='' ";
		...
		if( $_GET['document_cat_id'] > 0 ) {
			$sql[] = " documents.document_cat_id='{$_GET['document_cat_id']}' ";
		}
		...
		, GROUP_CONCAT( DISTINCT documents_index.class_id SEPARATOR '<~~>' ) as classes_ids
		
		, GROUP_CONCAT( DISTINCT students.full_name SEPARATOR '<~~>' ) as students_full_name
		, GROUP_CONCAT( DISTINCT documents_students.student_id SEPARATOR '<~~>' ) as students_ids
		...
		LEFT JOIN documents_students ON (documents.id = documents_students.document_id)
		LEFT JOIN students ON (students.id = documents_students.student_id AND students.school_id = documents.school_id )
		...
		if( $action == 'export' ) {
			...
			$excel->addField('document_cat_id', 'Category', 'style_documents_category');
...
<article class="module width_full">
	...
	<?php } ?>
		...
		<td><?php 
			if( $documentsCategories[ $row->document_cat_id ] ) {
				?><a href="<? echo $filename; ?>?<?php echo $queryStr; ?>document_cat_id=<? echo $row->document_cat_id; ?>"><?php echo $documentsCategories[ $row->document_cat_id ]['title']; ?></a><?php 
			}
		?>
		</td>
