...
$sortableLister = new lister("_sort_reverse.php?filename=".$filename."&table=slides");

$rank=getfield(getHTTP('rank'),"rank","slides");
...
switch ($action):
	//case "down":
		case "up": //reverse
		$strSQL="select * from slides WHERE rank='".($rank+1)."'" ;
		$objRS=mysql_query($strSQL);
		$total=mysql_num_rows($objRS);
		if($total>0){
			if ($row=mysql_fetch_object($objRS)){
				$strSQLord="update slides set rank='".($rank+1)."' WHERE rank='".$rank."'";
				mysql_query($strSQLord);
				$strSQLord="update slides set rank='".$rank."' WHERE id='".$row->id."'";
				mysql_query($strSQLord);
			}
		}
		else{
			$strSQL="select * from slides WHERE rank >'".$rank."'";
			$objRS=mysql_query($strSQL);
			$total=mysql_num_rows($objRS);
			if($total>0){
				if ($row=mysql_fetch_object($objRS)){
	...
	case "edit":
		...
		$strSQL="select * from slides where id IN(".implode(',',$ids).") {$limitation} order by rank DESC";
		$objRS=mysql_query($strSQL);
		$i=0;
		...
		for($i=0;$i<sizeof($ids);$i++){
			...
			if ($action=="editexe"):
				...
				if ( $Errors ){
					...
					if ( $result=file_upload('image','file1'.$i,'../uploads/slides/',$errorMsg[$j])){
						 $file1[$i]=$result['name'];
						if(empty($errorMsg[$j])){

							$Rimage = new SimpleImage();
							$Rimage->load('../uploads/slides/'.$file1[$i]);
							if( $Rimage->getWidth() > THUMB_WIDTH ) {
								$Rimage->resizeToWidth( THUMB_WIDTH );
							}
//							$Rimage->resize( THUMB_WIDTH, THUMB_HEIGHT);
							$Rimage->save('../uploads/slides/thumb/'.$file1[$i]);							
						}						 
					}
...
//							$Rimage->resize( THUMB_WIDTH, THUMB_HEIGHT );
					...
					if(!empty($errorMsg[$j])){// Upload Error
						...
						$q=mysql_query("SELECT max(rank) as max FROM slides");
...
//		if($j>0){
	...
	case "delete":
		...
		{
			if($row->image != '') {
				@unlink('../uploads/slides/'.$row->image);	
				@unlink('../uploads/slides/thumb/'.$row->image);	
			}

			$strSQL="DELETE FROM slides WHERE id = '".$row->id ."' LIMIT 1";
...
<article class="module width_full">
	<header><h3>Slides: <?php echo ucwords($action); ?> Record
...
<?php } ?>
			...
			<?php } ?>
				...
				<fieldset>
					<label>Image</label>
					<?php file_field('file1'.$i,'../uploads/slides/',$file1[$oldrecord[$i]]);?>
				</fieldset>
				<div class="clear"></div>
		</div>
	<footer></footer>
...
<?php 
		...
		if( !empty($keyword) )
		{
			...
			$sql[] = " (slides.title LIKE '%$keywordSQL%' ) ";
		}
	if( $enableStatus ) {
		switch($_GET['status']) {
			case 'active':
				$sql[] = " slides.status='active' ";
				break;
			case 'inactive':
//				$sql[] = " slides.status='' ";
				$sql[] = " slides.status<>'active' ";
		...
		if(isSchool) {
			$sql[] = " slides.school_id = '". isSchool ."' ";
		} else if($School) {
			$sql[] = " slides.school_id = '{$School['id']}' ";
		}
		$where = ( $sql ) ? " WHERE " . implode(' AND ', $sql ) : '';

		$strSQL="SELECT slides.* FROM slides $where";

		$objRS=mysql_query($strSQL);
		$total=@mysql_num_rows($objRS);
		$strSQL=makePages ($strSQL,$PageSize,$p, 'slides.rank desc');
		$objRS=mysql_query($strSQL);
...
<article class="module width_full">
	<header><h3 class="tabs_involved">Slides Manager</h3>
	<ul class="tabs">
...
<?php } ?>
    ...
    <tr id="tr_<? echo $row->id; ?>">
		...
		<td>
			...
			<div>
				<a href="<? echo $filename; ?>?<?php echo $queryStr; ?>action=edit&ids[]=<? echo $row->id; ?>">Edit slide</a>
			</div>
		</td>
		<?php if( !isSchool ) { ?>
...
<?php } ?>
		<td align="center"><?php echo scale_image("../uploads/slides/thumb/". $row->image, 100); ?></td>
