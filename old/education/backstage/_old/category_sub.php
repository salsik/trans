	...
	case "addexe":
		...
		for($i=0;$i<sizeof($ids);$i++){
			...
			if ($action=="editexe"){
				...
				if( !$category ) {
					$Errors[] = "Unable to fetch current sub-class data!!";
				}
			}
	...
	case "delete":
		...
		while ($row=mysql_fetch_object($objRS))
		{
			foreach($relatedArray as $tpl=>$eMsg) {
				...
				if( $q && mysql_num_rows($q)) {
					if( empty($errorMsg) ) {
						$errorMsg = "Some Records didn't affected, You may need to delete/update all {$eMsg} related to the deleted sub-class!!";
					}
					continue 2;
				}
			}
...
<article class="module width_full">
	<header><h3>Sub-Classes: <?php echo ucwords($action); ?> Record
...
//				$sql[] = " category_sub.status='' ";
		...
		if( $action == 'export' ) {
			...
			$excel = new export_excel("Sub-Classes");
			$excel->addField('title', 'Title');
			$excel->addField('description', 'Description');
			$excel->addField('cat_title', 'Class');
if( $enableStatus ) {
...
<article class="module width_full">
	<header><h3 class="tabs_involved">Sub-Classes Manager</h3>
	<ul class="tabs">
...
<?php } ?>
    ...
    <tr id="tr_<? echo $row->id; ?>">
		...
		<td>
			...
			<div>
				<a href="<? echo $filename; ?>?<?php echo $queryStr; ?>action=edit&ids[]=<? echo $row->id; ?>">Edit Sub-Class</a>
			</div>
		</td>
		<td>
			<div>
