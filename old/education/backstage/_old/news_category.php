...
//		if($j>0){
	...
	case "delete":
		...
		while ($row=mysql_fetch_object($objRS))
		{
			$q = mysql_query("SELECT * FROM `news` WHERE cat_id = '".$row->id ."' LIMIT 1");
			if( $q && mysql_num_rows($q) ) {
				if( empty($errorMsg) ) {
					$errorMsg = "Some Records didn't affected, You may need to delete/update all news related to the deleted category!!";
				}
				continue;
			}
