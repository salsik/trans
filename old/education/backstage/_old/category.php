		...
		while ($row=mysql_fetch_object($objRS))
		{
			foreach($relatedArray as $tpl=>$eMsg) {
				...
				if( $q && mysql_num_rows($q) ) {
					if( empty($errorMsg) ) {
						$errorMsg = "Some Records didn't affected, You may need to delete/update all {$eMsg} related to the deleted sub-classes!!";
					}
					continue 2;
				}
			}
		...
		<td>
			...
			<div>
				<a href="<? echo $filename; ?>?<?php echo $queryStr; ?>action=edit&ids[]=<? echo $row->id; ?>">Edit class</a>
				 - <a href="category_sub.php?cat_id=<? echo $row->id; ?>">Sub-Classes</a>
				 - <a href="category_sub.php?cat_id=<? echo $row->id; ?>&action=add">Add Sub-Class</a>
			</div>
		</td>
<?php if( $enableStatus ) { ?>
