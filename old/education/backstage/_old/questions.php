...
switch ($action):
	...
	//					$sql['reseller_id']= " `reseller_id`='0', ";
				...
				$_student = getDataByID('students', $student_id[$i], $limitationStudent);

				if( !$_student) {
					$Errors[] = "Missing Student!!";
				} else {
					$sql['student']= " `student_id`='{$_student['id']}', ";
				}
			}
...
?>
		...
		<div class="module_content inline">
			...
			<?php
					if( $action == 'edit') {
						$student = getDataByID('students', $_question['student_id'] );
					} else {
						$doctID = nor( $student_id[$oldrecord[$i]], $_GET['student_id'], true);
						$student = getDataByID('students', $doctID, $limitationStudent );
					}
					?>
					<span class="autocomplete" data-link="_get.php?from=students">
						<input class="input_short auto_name" type="text" name="student_name[]" value="<?php echo trim( $student['full_name']); ?>" />
						<input class="input auto_id" type="hidden" name="student_id[]" value="<?php echo $student['id']; ?>" />
					</span>
								</label>
							</td>
							<td><label></label></td>
...
<?php } ?>
		...
		Student: <span class="autocomplete" data-link="_get.php?from=students">
			<input class="input_short auto_name" type="text" name="student_name" value="<?php echo trim( $Student['full_name']); ?>" />
			<input class="input auto_id" type="hidden" name="student_id" value="<?php echo $Student['id']; ?>" />
		</span>
...
<?php 
		...
		if($Student) {
			$sql[] = " questions.student_id = '{$Student['id']}' ";
		}
		switch($_GET['status']) {
			case 'active':
...
//				$sql[] = " questions.status='' ";
		...
		$strSQL="SELECT questions.*, resellers.title as reseller_title, students.full_name as student_full_name
		FROM ( questions 
		LEFT JOIN resellers ON (resellers.id=questions.reseller_id) )
		LEFT JOIN students ON (students.id=questions.student_id)
		$where";
		...
		if( $action == 'export' ) {
			...
			$excel->addField('student_full_name', 'Student');
...
?>
	...
	<?php } ?>
		...
		<td>
			<div><b><? echo $row->student_full_name; ?></b></div>
		</td>
		<td>
	...
	<?php if( $replies ) { ?>
			...
			<div><b>From: </b><?php echo ($replies['from'] == 'reseller') ? "$row->reseller_title": "$row->student_full_name"; ?></div>
