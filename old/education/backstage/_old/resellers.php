	...
	case "delete":
		...
		while ($row=mysql_fetch_object($objRS))
		{
			...
			if(!mysql_query($strSQL) && empty($errorMsg)) {
				...
				mysql_query("DELETE * FROM `students_resellers` WHERE reseller_id='{$row->id}' ");
			}
		}
...
//			$sql[] = " catSub.sub_id='0' AND catSub.cat_id='{$Category['id']}' ";
		...
		$strSQL="SELECT resellers.*, GROUP_CONCAT( DISTINCT catSub.title SEPARATOR '<~~>' ) as cat_titles
		FROM resellers
		...
		) as catSub ON (catSub.sub_id = resellers_index.sub_id AND catSub.sub_id = resellers_index.sub_id)
...
<article class="module width_full">
    ...
    <tr id="tr_<? echo $row->id; ?>">
		...
		<?php if( $row->contact_email ) { ?>
			<div><b>Email:</b> <? echo $row->contact_email; ?></div>
		<?php } ?>
		</td>
		<td>
			<div>
