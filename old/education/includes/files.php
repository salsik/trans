...
function file_upload ($file_orig_type,$myvar,$path,&$status,$max_size=''){ 
 ...
 if (!$real_name){
  ...
  return false;
	  ...
	  }elseif($file_orig_type == "document"){	  	$type_of_file_must_be = "application";	  	$images_extentions_array = array("pdf","doc","dox","txt");	  }elseif($file_orig_type == "documents"){	  	$type_of_file_must_be = TRUE;	  	$images_extentions_array = array("pdf","doc","docx","txt", "xlsx", "xls", "ppt", "pptx", "jpg", "jpeg", "png", "gif");	  }elseif($file_orig_type == "pdf"){	  	$type_of_file_must_be = "application";	  	$images_extentions_array = array("pdf");
	  ...
	  if(!in_array($ext_name,$images_extentions_array)){
	  	if($file_orig_type == "image"){
	  		...
	  		$status.="Only documents is allowed (.pdf .doc .docx .txt)";	  	}elseif($file_orig_type == "documents"){	  		$status.="Only documents is allowed (.pdf .doc .docx .txt .xlsx .xls .ppt .pptx .jpg .jpeg .png .gif)";	  	}elseif($file_orig_type == "pdf"){	  		$status.="Only documents is allowed (.pdf)";
	  	}elseif($file_orig_type == "media"){
	  	...
	  	list($file_type_new) = explode("/",$file_type);
	  	  ...
	  	  if($file_type_new !== $type_of_file_must_be && !$flv_application && $type_of_file_must_be !== true){
	  	  	if($file_orig_type == "image"){
		  	  	$status.="Please insert a valid image";
	  	  	}elseif($file_orig_type == "video"){
		  	  	$status.="Please insert a valid video";
	  	  	}elseif($file_orig_type == "document"){
		  	  	$status.="Please insert a valid file";	  	  	}elseif($file_orig_type == "documents"){		  	  	$status.="Please insert a valid file";
	  	  	}elseif($file_orig_type == "media"){
