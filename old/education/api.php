...
$previsionDefaultImage = previsionDefaultImage();
...
$newsCategories = getSectionsCategoriesFromIndex( 'news_category', $account['schoolIDs'] );
$documentsCategories = getSectionsCategoriesFromIndex( 'documents_category', $account['schoolIDs'] );

if( $account ) {
	$galleryCategories = getSectionsCategories( 'gallery_category', $account['schoolIDs']);
}
...
//print_r($account['schools'][ $_REQUEST['school_id'] ]);
		...
		$def_img = previsionDefaultImage($_school['image']);
...
//					'school_id' => $row['school_id'],
					'date' => date('Y-m-d', $row['time']),
					'image' => ($row['image']) ? $_SITE_PATH_ . 'uploads/'.$table.'/' . $row['image'] : $def_img,
					'video' => $row['video'],
				);
				...
				elseif( $table == 'education_news') {
					...
					$response['data']['token'] = md5($_REQUEST["key"]);
				}
			...
			if( mysql_num_rows($q)) {
				while($row = mysql_fetch_assoc($q)) {
					...
					$response['data'][] = array(
						...
						'title' => $row['title'],
						'description' => summarize($row['description'], 20),
						'image' => ($row['image']) ? $_SITE_PATH_ . 'uploads/polls/thumb/' . $row['image'] : $def_img,
					);
				}
			}
		}
...
//			$response['error'] = "$strSQL " . mysql_error();
			...
			$response['poll'] = array(
				...
				'image' => ($poll['image']) ? $_SITE_PATH_ . 'uploads/polls/thumb/' . $poll['image'] : $def_img,
			);
			// fix in app
...
//			$response['error'] = mysql_error();
			...
			if( !mysql_num_rows($q)) {
				...
				$response['competition'] = array(
					...
					'image' => ($competition['image']) ? $_SITE_PATH_ . 'uploads/competitions/thumb/' . $competition['image'] : $def_img,
				);
				// fix in app
				...
				if( $q && mysql_num_rows($q)) {
					while($row = mysql_fetch_assoc($q)) {
						$response['users'][] = array(
							'id' => $row['id'],
							'full_name' => $row['title'],
							'image' => ($row['image']) ? $_SITE_PATH_ . 'uploads/users/thumb/' . $row['image'] : $def_img,
						);
					}
				}
			...
			if( mysql_num_rows($q)) {
				while($row = mysql_fetch_assoc($q)) {
					...
					$response['data'][] = array(
						...
						'image' => ($row['image']) ? $_SITE_PATH_ . 'uploads/questions/thumb/' . $row['image'] : $def_img,
					);
				}
			}
		}
			...
			if( mysql_num_rows($q)) {
				...
				if( !($_page > 1) ) {
					$response['question'] = array(
						...
						'image' => ($question['image']) ? $_SITE_PATH_ . 'uploads/questions/' . $question['image'] : $def_img,
					);
				}
				// fix in app
...
//				if( $_school['email']) {
					$contactData = array_map('htmlClean', $_REQUEST);
					
					include_once( BASE_DIR . "_emails.php");
					
					send_school_contact_email( $_school, $contactData);
//				}
...
//	var_dump( $row );
		...
		$newsCategories = getSectionsCategoriesFromIndex( 'news_category', $row['schoolIDs'] );
		$documentsCategories = getSectionsCategoriesFromIndex( 'documents_category', $row['schoolIDs'] );
		$galleryCategories = getSectionsCategories( 'gallery_category', $row['schoolIDs']);                
...
function buildLoginResponse($key, $account) {
	...
	$response['categories_gallery'] = array_values( $galleryCategories );
        $response['notifications'] = getPushNotifications( $account, $account['school']['id'] );
...
//		'username' => $account['username'],
		...
		'image' => ($account['image']) ? BASE_URL . 'uploads/users/thumb/' . $account['image'] : $previsionDefaultImage,
	);
	...
	if( is_array( $account['schools'] )) {
		foreach( $account['schools'] as $school) {
			...
			if( $q && mysql_num_rows( $q )) {
				while( $row = mysql_fetch_assoc( $q )) {
					if($row['image']){
						$slides[] = BASE_URL . 'uploads/slides/' . $row['image'];
					}
				}
			}
			...
			$response['schools'][] = array(
				...
				'title' => $school['title'],
				'slides' => $slides,
				'logo' => ($school['image']) ? BASE_URL . 'uploads/schools/' . $school['image'] : $previsionDefaultImage,
			);
		}
	}
...
//				'info_mobile' => $student['info_mobile'],
				'photo' => ($student['image']) ? BASE_URL . 'uploads/students/' . $student['image'] : $previsionDefaultImage,
			);
		
			if($student['info_mobile'] == $account['username']) {
				if(empty($response['account']['address']) && !empty($student['info_address'])) {
					$response['account']['address'] = $student['info_address'];
				}
				if(empty($response['account']['email']) && !empty($student['info_email'])) {
					$response['account']['email'] = $student['info_email'];
				}
				if(empty($response['account']['mobile']) && !empty($student['info_mobile'])) {
					$response['account']['mobile'] = $student['info_mobile'];
				}
			}
		}
	}
...
function saveInsert($string) {
	return mysql_real_escape_string( htmlClean( $string ) );
}
...
function previsionDefaultImage($image = '') {
	if($image) {
		return BASE_URL . 'uploads/schools/thumb/'.$image;
	}
	return BASE_URL . 'prevision-education.png';;
}