...
{
	$sql = getStudentLogin('key', $_REQUEST["key"]);
	...
	if ( $q && mysql_num_rows($q) ) 
	{
		...
		$account['student_login_id'] = intval($account['student_login_id']);
		
		$reseller = getDataByID('resellers', $account['student_reseller_id'], " status='active' ");
...
if( $account && time() - $account['student_login_time_update'] > _login_update_time) {
	$sql = "UPDATE `students_logins`
		SET `time_update`='".time()."'
		WHERE id='{$account['student_login_id']}'
		LIMIT 1 ";
	$q = mysql_query($sql);
	if( $q ) {
		$account['student_login_time_update'] = time();
	}
}
...
switch( $action ) {
	...
	case 'banners':
		...
		foreach($banners as $k=>$v) {
			...
			if( $account && $_BannersZones[ $k ]['reseller'] ) {
...
//					$link = $row['link'];
					...
					$maxWidth = ($_BannersZones[ $k ]['maxWidth']) ? ' class="maxWidth" ' : '';
...
//					WHERE id='{$account['id']}' 
//					LIMIT 1");
				$sql = "DELETE FROM `students_logins`
					WHERE id='{$account['student_login_id']}'
					LIMIT 1 ";
				$q = mysql_query($sql);
//				echo $sql;
...
//	var_dump($_REQUEST);
				if( $table == 'news') {
					...
					if( $news_cat_id > 0 ) {
						$where = " news.news_cat_id = '{$news_cat_id}' ";
					}
				}
//		var_dump($where);
...
//		echo $strSQL;
				...
				$q = mysql_query("$strSQL ORDER BY {$table}.rank DESC LIMIT $_offset, $_limit");
						...
						if( $q && mysql_num_rows($q)) {
							while($row = mysql_fetch_assoc($q)) {
								$response['students'][] = array(
									...
									'image' => ($row['image']) ? $_SITE_PATH_ . 'uploads/students/thumb/' . $row['image'] : '',
								);
							}
						}
...
//				$error = print_r( $_REQUEST , true);
			...
			if( empty( $error ) ) {
				...
				$sql = getStudentLogin('login', $username, $password );
//echo $sql;
				$q = mysql_query ($sql);
				if ( !$q  ) {
					$error = "Username or password not valid!!!";
				}
				else if ( $q && mysql_num_rows($q) ) {
					$row = mysql_fetch_assoc ($q);
	
					$reseller = getDataByID('resellers', $row['student_reseller_id'], " status='active' ");
	
					if( $reseller ) {

						$key = md5($row['id'] . $row['email'] . $row['password'] . $api_hash);

						$q = mysql_query("REPLACE INTO students_logins SET 
						...
						"); // INSERT
						if( $q ) {
							$row['student_login_id'] = mysql_insert_id();
...
//							$response['notifications'] = getPushNotifications( $row );
							...
							$q = mysql_query("INSERT INTO students_logins_devices SET 
								`student_id`='{$row['id']}' 
								, `source`='".saveInsert( $_REQUEST['source'] )."'
								, `regid`='".saveInsert( $_REQUEST['regid'] )."'
								, time='".time()."'
...
function buildLoginResponse($key, $Categories, $account, $reseller) {
	...
	$response['news_categories'] = $newsCategories;
	...
	$response['account'] = array(
		...
		'email' => $account['email'],
		'image' => ($account['image']) ? $_SITE_PATH_ . 'uploads/students/thumb/' . $account['image'] : '',
	);
...
function getPushNotifications( $account ) {
	$notifications = array();
	$sql = "SELECT * FROM students_logins WHERE id='{$account['student_login_id']}' LIMIT 1";
	$q = mysql_query( $sql );
	if( $q && mysql_num_rows($q)) {
		$row = mysql_fetch_assoc($q);
		...
		mysql_query( "UPDATE students_logins SET 
		...
		WHERE id='{$account['student_login_id']}' LIMIT 1
		" );
	}
